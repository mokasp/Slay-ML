...
def direction(image, face_mesh, segmentation):
    """ finds which direction person in photo is facing to determine which hair region to extract """
    # loading in image
    img = image
    rgb_img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

    # get segments and facemesh
    ...
    if face_results.multi_face_landmarks:
        ...
        # calculating the horizontal distance between the nose tip and each eye
        nose_to_left_eye = abs(fx - le_x)
        nose_to_right_eye = abs(fx - re_x)

        # calculating vertical distances (y-coordinates)
        nose_to_left_eye_y = abs(fy - le_y)
        nose_to_right_eye_y = abs(fy - re_y)

        # setting thresholds for detecting left, right, and forward
...
def combine_lab_values(top_lab, bottom_lab):
    """ combines lab values for top and bottom lip to get average lip color """
    lab_combined = [
...
def get_lab_vector(image):
    """ gets the LAB vector from a single region """
    img = image
    ...
    # converting to human-interpretable Lab scale
...
def extract_hair(image, dir, face_mesh, segmentation):
    """ extracts hair regions """
    input_regions = []
    region_names = ["00_side_hair", "01_top_hair"]
    img = image
    rgb_img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

    # get segmentations and facemesh
    segmentation_results = segmentation.process(rgb_img)
    binary_mask = (segmentation_results.segmentation_mask > 0.1).astype(np.uint8)
    face_results = face_mesh.process(rgb_img)
...
def extract_regions(image, input_regions, region_names, face_mesh):
    """ extracts facial regions from frame """
    output_regions = []
    ...
    if results.multi_face_landmarks:
        for face_landmarks in results.multi_face_landmarks:
            ...
            # separate cropped images of each region
...
def process_regions(image, hair_face_mesh, segmentation, face_mesh):
    """ process hair and facial regions and extract mean color vectors """
    lab_vectors = []
    lip_lab_vectors = []

    # check direction person is facing and get appropriate hair regions
    ...
    j = 0
    # get the lab vectors for all regions to process
    ...
    # if region extraction failed, return empty lists
    if len(lab_vectors) != 12:
        return [], []
    
    # calculate combined lab values
...
def load_data(image):
    """ loads data for training and prediction """
    X = []
    y = []
...
def normalize_vectors(data, loaded_data=False):
    """ normalizes vectors before they are used for prediction """
    if loaded_data:
        X = data['X']
        y = data['y']
    else:
